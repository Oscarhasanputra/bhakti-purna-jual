import { Component, ViewEncapsulation, Output, EventEmitter } from '@angular/core';
import { Router } from '@angular/router';
import { BUSY_CONFIG_DEFAULTS, IBusyConfig } from 'ng-busy';

import { LookUpBassReviewClaimService } from './lookupbassreviewclaim.service';
import { GlobalState } from '../../../../../../global.state';

@Component({
    selector: 'lookupbass-reviewclaim',
    encapsulation: ViewEncapsulation.None,
    templateUrl: './lookupbassreviewclaim.html',
    styleUrls: ['./lookupbassreviewclaim.scss']
})
export class LookUpBassReviewClaim {
    busyLoaderEvent: IBusyConfig = Object.assign({}, BUSY_CONFIG_DEFAULTS);
    display: boolean = false;
    datas: any[];
    searchBass: String = "";
    selectedBass: any;

    @Output() modalData = new EventEmitter<String>();

    constructor(protected service: LookUpBassReviewClaimService, protected router: Router, public global: GlobalState) {
        this.busyLoaderEvent.message = `Please Wait...`;
    }

    showModal() {
        this.display = true;
        this.loadDataBass();
    }

    textChange() {
        // console.log(this.searchBass);
        this.modalData.emit(this.searchBass);
    }

    onRowSelect(event) {
        this.searchBass = event.data.KODE_BASS;
        this.modalData.emit(this.searchBass);
        this.display = false;
    }

    loadDataBass() {
        if (this.global.Decrypt('mAuth').TYPE == "Cabang") {
            this.busyLoaderEvent.busy = [this.service.getBassListUnderCabang(this.global.Decrypt('mAuth').KODE_BASS)
                .then(
                data => {
                    this.datas = data;
                },
                err => {
                    if (err._body.indexOf("TokenExpiredError") && err.status == 500) {
                        alert("Your Token has expired, please login again !")
                        sessionStorage.clear();
                        this.router.navigate(['/login']);
                    }
                })]
        } else {
            this.busyLoaderEvent.busy = [this.service.getBassList("")
                .then(
                data => {
                    this.datas = data;
                },
                err => {
                    if (err._body.indexOf("TokenExpiredError") && err.status == 500) {
                        alert("Your Token has expired, please login again !")
                        sessionStorage.clear();
                        this.router.navigate(['/login']);
                    }
                })]
        }
    }
}