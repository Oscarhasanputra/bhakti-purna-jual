...
export class ClaimReportService {
    ...
    getClaimReports(kode_dealer, tgl_awal, tgl_akhir) {
        ...
        if (kode_dealer == '') {
            return this.http.get<any>(this.global.GlobalUrl + '/reportClaim' + '/' + tgl_awal + '/' + tgl_akhir, {headers});
        } else {
            return this.http.get<any>(this.global.GlobalUrl + '/reportClaim/' + kode_dealer + '/' + tgl_awal + '/' + tgl_akhir, {headers});
