import { Component, ViewChild, ViewEncapsulation, Output, EventEmitter, OnInit } from '@angular/core';
import { SelectItem } from 'primeng/api';
import { ModalDirective } from 'ngx-bootstrap/modal';

import { BrowseListBassReportService } from './browseListBassReport.service';
import { Subscription } from 'rxjs';
import { Router } from '@angular/router';
import { GlobalState } from '../../../../../global.state';

@Component({
  selector: 'browselistbassreport',
  encapsulation: ViewEncapsulation.None,
  styleUrls:['./browseListBassReport.component.scss'],
  templateUrl:'./browseListBassReport.component.html'
})
export class browseListBassReport {

  public reportServiceLists: any;
  public kode_dealer;
  public tglAwal: any;
  public tglAkhir: any;
  public listBass: listBass[];
  public sKodeBass: any;
  sStorage: any;
  display: boolean = false;
  showPilihKodeBass: boolean = false;
  @Output() showPilihKodeBassChild = new EventEmitter<boolean>();
  @Output() kodeBassChild = new EventEmitter<string>();

  showDialog() {
    this.display = true;
  }

  @ViewChild('childModal') childModal: ModalDirective;

  showChildModal(): void {
    this.childModal.show();
  }

  hideChildModal(): void {
    this.childModal.hide();
  }

  constructor(private browseListBassReportService: BrowseListBassReportService, protected router: Router,
    public global: GlobalState) {

    this.sStorage = this.global.Decrypt('mAuth')

    if (this.sStorage.TYPE == "Cabang") {
      this.showPilihKodeBass = true;

      this.browseListBassReportService.getBassListUnderCabang(this.sStorage.KODE_BASS).then(
        data => {
          this.listBass = data;
        },
        err => {
          if (err._body == 'You are not authorized' || err.status == 500) {
            alert("Your Token has expired, please login again !")
            sessionStorage.clear();
            this.router.navigate(['/login']);
          }
        }
      );
    } else if (this.sStorage.KODE_BASS == this.global.Decrypt('mParameter').BASS_PUSAT) {
      this.showPilihKodeBass = true;

      this.browseListBassReportService.getBassList(this.sStorage.KODE_BASS).then(
        data => {
          this.listBass = data;
        },
        err => {
          if (err._body == 'You are not authorized' || err.status == 500) {
            alert("Your Token has expired, please login again !")
            sessionStorage.clear();
            this.router.navigate(['/login']);
          }
        }
      );
    } else {
      this.showPilihKodeBass = false;

      this.browseListBassReportService.getBassList(this.sStorage.KODE_BASS).then(
        data => {
          this.listBass = data;
        },
        err => {
          if (err._body == 'You are not authorized' || err.status == 500) {
            alert("Your Token has expired, please login again !")
            sessionStorage.clear();
            this.router.navigate(['/login']);
          }
        }
      );
    }

    this.execemit();
  }

  onBlurMethod() {
    this.execemit();
  }

  onRowSelect(event) {
    this.sKodeBass = event.data.KODE_BASS;
    this.display = false;
    this.execemit();
  }

  execemit() {
    this.showPilihKodeBassChild.emit(this.showPilihKodeBass);
    this.kodeBassChild.emit(this.sKodeBass);
  }

}

export interface listBass {
  KODE_BASS;
  NAMA_BASS;
}
